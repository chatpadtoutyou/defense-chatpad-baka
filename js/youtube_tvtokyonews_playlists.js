...
const youtubeTvtokyonews = setInterval(() => {
    ...
    const tag = $(document).find(`.yt-lockup-metadata-view-model-wiz__heading-reset[title="${item}"]`);
    console.log(tag);

    if (tag.length > 0) {
        const href = tag.siblings('div').find('a').attr('href');
        const url = `https://www.youtube.com${href}`;
        const title = item;
        const targetTag = $('#sub-menu');
